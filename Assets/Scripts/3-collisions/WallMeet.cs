...
public class WallMeet : MonoBehaviour
{
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (other.tag == "Player" || (other.tag == "Laser" && gameObject.name=="topWall") || (other.tag == "Enemy" && gameObject.name == "bottomWall"))
        {
            Debug.Log("Right wall triggered by player");
        ...
        {
            Debug.Log("Right wall triggered by " + other.name);
        }
    }
